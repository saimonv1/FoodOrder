...
const authenticateTokenPersonalUsername = async(req, res, next) => {
  ...
  await jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, async (err, user) => {
    if (err) return res.status(403).json({ message: "Forbidden" });
    if(!await utils.verifyUserUsername(user.username, req.params.userName)) {
        return res.status(403).json({ message: "Forbidden" });
    }
    req.user = user;
    next();
...
module.exports = { authenticateTokenAdmin, authenticateTokenPersonal, authenticateTokenPersonalUsername };
